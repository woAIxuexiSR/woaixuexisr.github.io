...
由于各向异性在相邻单元间变化，不能简单地用各向异性的质心与质量替换标准 CVT 梯度。作者通过可微裁剪算法把高维嵌入曲面分解为单纯形三角面片，用闭式表达求 $$E_{hd}$$，并用链式法则与反向模式自动微分求梯度。注意其面片面积 $$\vert T\vert $$ 依赖于能量项（不同于仅用 Heron 公式而无度量的 6D CVT），因此需对 Heron 公式也求导。优化用 L-BFGS，RVD 计算用 Geogram，自动微分用 Stan Math Library。最后按重心坐标把 RVD 回投到三维得到各向异性 RVD 与对偶网格；若回投出现翻转元素，则用可终止算法插点修复。
